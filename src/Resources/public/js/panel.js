...
opendxp.registerNS("opendxp.plugin.objectmerger.panel");
opendxp.plugin.objectmerger.panel = Class.create({
    ...
    activate: function () {
        ...
        tabPanel.setActiveItem("opendxp_plugin_objectmerger_panel_" + this.id);
    },
    ...
    toggle: function () {
        this.panel.close();
        new opendxp.plugin.objectmerger.panel(this.oid2, this.oid1);
    },
    ...
    getTabPanel: function () {
        ...
        if (!this.panel) {
            this.panel = new Ext.form.FormPanel({
                ...
                id: "opendxp_plugin_objectmerger_panel_" + this.id,
            ...
            this.panel.on("destroy", function () {
                opendxp.globalmanager.remove("plugin_objectmerger_" + this.id);
                var accordion = Ext.getCmp("opendxp_panel_tree_left");
                accordion.expand();
            }.bind(this));
    ...
    panelActivated: function (language, thePanel) {
        ...
        for (var i = -1; i < items.length; i++) {
            ...
            if (item) {
                ...
                if (item.type) {
                    iconClass = this.executeFunctionByName("opendxp.object.classes.data." + item.type + ".prototype.getIconClass", window);
                    typeName = this.executeFunctionByName("opendxp.object.classes.data." + item.type + ".prototype.getTypeName", window);
                }
            } else {
                typeName = t("plugin_objectmerger_go");
                iconClass = "opendxp_icon_edit";
                handler = opendxp.helpers.openObject.bind(this, this.data.o1id, "object");
            }
            ...
            var gridHandler = function (item, callback) {
                ...
                var window = new opendxp.plugin.objectmerger.grideditor(item, itemData, callback);
                window.show();
            ...
            if (hasHandler) {
                if (item && item.isdiff) {
                    ...
                    applyButton.setHandler(opendxp.helpers.openObject.bind(this, this.data.o2id, "object"));
                }
            }
    ...
    getTabForLanguage: function (language) {
        ...
        var formForLanguage = new Ext.Panel({
            title: title,
            iconCls: "opendxp_icon_language_" + language.key.toLowerCase(),
    ...
    saveComplete: function (response) {
        var data = Ext.decode(response.responseText);
        if (data.success) {
            const pluginObjectMergerPostMerge = new CustomEvent(opendxp.events.pluginObjectMergerPostMerge, {
                detail: {
                    data: data
                }
            });
            ...
            opendxp.helpers.showNotification(t("success"), t("your_object_has_been_saved"), "success");
        } else {
            opendxp.helpers.showNotification(t("error"), t(data.message), "error");
        }
    }
});
