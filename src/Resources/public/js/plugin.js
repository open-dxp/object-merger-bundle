...
opendxp.registerNS("opendxp.plugin.objectmerger");

opendxp.plugin.objectmerger = Class.create({
    getClassName: function (){
        return "opendxp.plugin.objectmerger";
    },

    initialize: function(){
        document.addEventListener(opendxp.events.onPerspectiveEditorLoadPermissions, this.perspectiveEditorLoadPermissions.bind(this));
    ...
    createNavigationEntry: function (e) {
        ...
        if(e.type === opendxp.events.opendxpReady) {
            const menu = opendxp.globalmanager.get('layout_toolbar').extrasMenu;
    ...
    showDiff: function(response) {
        ...
        if (data.success) {
            ...
            opendxp.globalmanager.add("plugin_objectmerger", new opendxp.plugin.objectmerger.panel(data.oid1, data.oid2));
        } else {
            Ext.MessageBox.show({
                title:t('error'),
...
var objectmergerPlugin = new opendxp.plugin.objectmerger();
